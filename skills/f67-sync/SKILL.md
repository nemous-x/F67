...
1. Determine what changed since the last sync: use `git diff --name-status` against the last synced commit recorded in `state/execution-history.json` (fall back to comparing related-files.json against the working tree).
2. Classify changes: files added/moved/deleted, domains affected (via file-graph.json), completed workflow artifacts not yet folded into memory.

## Pipeline

1. Dispatch `f67-memory-evolver` with the change classification. It:
   - Updates `related-files.json` and all graphs for moved/added/deleted files.
...
3. Record the synced commit hash in `state/execution-history.json`.
